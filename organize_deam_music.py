...
def map_valence_arousal_to_emotion(valence, arousal):
    ...
    if valence >= 6 and arousal >= 6:
        return random.choice(['happy', 'surprise'])  # 两个都合理
    elif valence >= 6 and arousal <= 4:
        return 'calm'
    elif valence <= 4 and arousal >= 6:
        return random.choice(['angry', 'fear'])  # 高唤醒但低愉悦
    elif valence <= 4 and arousal <= 4:
        return random.choice(['sad', 'disgust'])  # 低唤醒且负面
    else:
        return 'neutral'
...
def organize_deam_by_emotion(
    ...
    for _, row in df.iterrows():
        song_id = f"{int(row['song_id'])}.mp3"  # 不使用 zero-padding
    ...
    print(f"\n✅ Done! Copied {count} songs into {output_dir}/[emotion]/ folders.")
